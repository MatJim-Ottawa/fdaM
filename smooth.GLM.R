smooth.GLM <- function(argvals, y, fdParobj, weight=NULL, fdnames=list('arguments','replications','variables'), covariates=NULL, 
  ...
  argchk = argcheck(argvals)
  
  argvals = argchk$argvals
  n       = argchk$n
  ...
  # Looks like a bug in fdPar ? Does't passs check for inheriting from 'fd'
  ...
  if (n >= nbasis || lambda > 0)
  {
    ...
    if (ndim < 3)
    {
      ...
      result = glm.fda(basismat, y, family, lamRmat, wtvec)
      coef  = result$Bvec
      dev   = result$Deviance
    }
    else
    {
      ...
      for (ivar in 1:nvar)
      {
        ...
        coef[,,ivar]  = result$Bvar
        dev[,ivar]    = result$Deviance
        #             stats{,ivar}  = statsi
      }
    }
