...
for i=weekindex
    ...
    axis([-150,-50,40,90,-2,10])
    view(AZ,EL)
    caxis([-2,10])
    shading interp
    title(['\fontsize{24} Precipitation for Week ',num2str(i),' '])
    pause(1)
end
...
csvwrite('mesh_E.csv',Canada_mesh_E)
csvwrite('mesh_P.csv',Canada_mesh_P)
csvwrite('mesh_T.csv',Canada_mesh_T)

csvwrite('mesh_Lat.csv',Lat)
csvwrite('mesh_Lng.csv',Lng)


csvwrite('data_prec.csv',precdata)
